...
def add_department(session: Session):
    ...
    while not unique_chair_name or not unique_abbreviation or not unique_office or not unique_description or not unique_name:
...
def add_section_student(sess):
    ...
    while not is_enrolled:
        ...
        if not is_enrolled:
            print(f"{student.firstName} {student.lastName} is already enrolled in {section.departmentAbbreviation} "
                  f"{section.courseNumber} section {section.sectionNumber}.  Try again.")
...
def add_student_section(sess):
    ...
    while not is_enrolled:
        ...
        if not is_enrolled:
            print(f"{student.firstName} {student.lastName} is already enrolled in {section.departmentAbbreviation} "
                  f"{section.courseNumber} SEC{section.sectionNumber}.  Try again.")
...
def add_student_PassFail(sess):
    ...
    while not is_enrolled:
        ...
        is_enrolled = enrollment_count == 0
        if not is_enrolled:
            print("Student already enrolled in a section of the same course, semester, and year.  Try again.")
...
def add_student_LetterGrade(sess):
    ...
    :param sess:
    :return:
    1. Make sure that we do not already have an Enrollment of any sort for that combination of
    Student and Section.
    2. Prompt the user for the grade.
    """
    student: Student
    section: Section
    is_enrolled: bool = False
    while not is_enrolled:
        ...
        if not is_enrolled:
            print("Student already enrolled in a section of the same course, semester, and year. Try again.")
    minSatisfactory = input("Enter grade (A, B, C, D, F): ")
    letter_grade = LetterGrade(section, student, minSatisfactory)
...
def delete_student(session: Session):
    ...
    if n_sections > 0:
        print(
            f"Sorry, this student is enrolled in {n_sections} sections. UnEnroll this "
            f"student from all sections then come back here to delete the student.")
    else:
...
def delete_section(session: Session):
    ...
    n_students = session.query(Enrollment).filter(Enrollment.sectionCode == section.code).count()
    if n_students > 0:
        print(
            f"Sorry, this section has {n_students} student(s) enrolled. UnEnroll all students "
            f"from this section then come back here to delete this section.")
    else:
...
def delete_student_section(sess):
    ...
    1. Validate that when you unenroll a student from a Section, that any instance of PassFail or
    LetterGrade that are the child of that Enrollment are also deleted.
    """
    print("Prompting you for the student and the class section they are dropping.")
    student = select_student(sess)
    section = select_section(sess)
    pk_count: int = count_student_section(sess, student, section)
    unique_student_section = pk_count == 0
    if unique_student_section:
        print(f"{student.firstName} {student.lastName} was not enrolled in the given section, therefore they cannot be un-enrolled")
    if not unique_student_section:
        student.remove_section(section)
        print("Student successfully un-enrolled")
...
def delete_section_student(sess):
    ...
    :return:        None
    """
    print("Prompting you for the class section and the student dropping that class.")
    section = select_section(sess)
    student = select_student(sess)
    pk_count: int = count_student_section(sess, student, section)
    unique_section_student = pk_count == 0
    if unique_section_student:
        print(f"Course {section.courseNumber}, section {section.sectionNumber} does not have {student.firstName} {student.lastName} on the roster")
    if not unique_section_student:
        section.remove_student(student)
        print("Student successfully un-enrolled from course")
...
def list_enrollment(sess: Session):
    ...
    1. Validate that the list_enrollment method works on LetterGrade Enrollment instances as well as
    PassFail instances of Enrollment
    """
...
def boilerplate(sess):
    ...
    :return:        None
    """
    department: Department = Department('Computer Engineering Computer Science', 'CECS', 'Shelly', 'VEC', 345, 'very difficult major indeed')
    ...
    section2: Section = Section(course1, 2, 'Fall', 2023, 'ECS', 416, 'MW', time(10, 0, 0), 'Brown')
    student1.add_section(section1)
    # student1.add_section(section2) #should return an error if done correctly
